...
class HFModelConfig(BaseConfig):
    ...
    # Selective PEFT: path to profiling mask JSON (from profiling/generate_mask_*.py).
    # - With LoRA (lora_rank > 0): overrides target_modules with mask's active linear weights
    # - Without LoRA (FFT): freezes mask's frozen_params via requires_grad=False
    selective_mask_path: Optional[str] = None
