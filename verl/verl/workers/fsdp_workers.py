...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if self._is_lora:
            ...
            if lora_adapter_path is not None:
                ...
                # Selective LoRA: if mask provided, override target_modules
                selective_mask_path = self.config.model.get("selective_mask_path")
                if selective_mask_path is not None:
                    from verl.utils.selective_peft import load_mask, mask_to_target_modules
                    mask = load_mask(selective_mask_path)
                    target_modules_list = mask_to_target_modules(mask, model=actor_module, verbose=(self.rank == 0))
                else:
                    target_modules_list = convert_to_regular_types(self.config.model.target_modules)
        ...
        # Selective Freeze FFT: freeze params from mask before FSDP wrap
        selective_mask_path = self.config.model.get("selective_mask_path")
        if not self._is_lora and selective_mask_path is not None:
            from verl.utils.selective_peft import load_mask, apply_selective_freeze
            mask = load_mask(selective_mask_path)
            apply_selective_freeze(actor_module, mask, verbose=(self.rank == 0))
            self.use_orig_params = True  # Required for FSDP with mixed requires_grad
        ...
        if role == "actor" and optim_config is not None:
            ...
            # Filter to trainable params only (supports selective freeze)
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        get_torch_device().manual_seed(rollout_config.seed + gen_dp_rank + 1000)
